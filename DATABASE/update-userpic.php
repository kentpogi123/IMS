...
if($file_data['tmp_name'] !== ''){
    ...
    if($check) {
 	...
 	try {
		...
		$sql = "UPDATE users SET email=?, first_name=?, last_name=?, password=?, user_pic=?, update_at=? WHERE id=?";
		include('connect.php');
    ...
    $existingUser = $selectStmt->fetch(PDO::FETCH_ASSOC);
		...
		$stmt->execute([$email, $first_name, $last_name, $password, $file_name_value, date('Y-m-d h:i:s'), $userid]);	
