...
try {
	$sql = "UPDATE tablewares SET PC_tableware=?, tableware_type=?, quantity=?, g_condition=?, missing=?, damage=?, unit=?, updated_at=? WHERE id=? ";
	include('connect.php');
	...
	$stmt->execute([$PC_tableware, $tableware_type, $quantity, $g_condition, $missing, $damage, $unit,  date('Y-m-d h:i:s'), $tid]);
