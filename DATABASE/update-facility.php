...
session_start();
	date_default_timezone_set('Asia/Manila');
 	...
 	try {
		...
		$sql = "UPDATE facilities SET facility_name=?, state=?, description=?, status=?, updated_at=? WHERE id=?";
		include('connect.php');
    ...
    $existingFacility = $selectStmt->fetch(PDO::FETCH_ASSOC);
		...
		$stmt->execute([$facility_name, $state, $description, $status, date('Y-m-d h:i:s'), $facilityid]);	
