<?php
  ...
  $_SESSION['table'] = 'activitylogs';
   $activitylogs = include('DATABASE/show.php');
	...
	<div id="dashboardMainContainer">
		<?php include('partials/app-sidebar.php') ?>
			...
			<?php 
								...
								<div class="printcss">
				<button  onclick="deleteActivityLogs2()">Delete Activity Logs</button>
				</div>
										...
										<tbody id="showdata">
											...
											<tr>
												...
												<td class="lastName"><?php 
													$acid = $activitylog['user_id'];
													$stmt = $conn->prepare("SELECT * FROM  users WHERE id=$acid");
													$stmt->execute();
												...
												<td class="lastName"><?=$activitylog['action_made'] ?> </td>
												<td class="lastName"><?=date('M d, Y @ h:i:s A', strtotime( $activitylog['created_at']))?></td>
												
											</tr>
										     <?php } ?> 
										</tbody>
									</table>
									<p class="userCount"><?= count($activitylogs) ?> activitylogs</p>
								</div>
							</div>
						</div>
					</div>
